...
const EditorLayout = () => {
  ...
  const parseEditorContent2JSON = async (format: EFormat, content: string) => {
    switch (format) {
      case EFormat.HTML:
        return JSON.stringify(await editor.tryParseHTMLToBlocks(content))
      case EFormat.MARKDOWN:
        return JSON.stringify(await editor.tryParseMarkdownToBlocks(content))
      case EFormat.JSON:
        return content
    }
  }
    ...
    if (port) {
      ...
      port.onmessage = async (event: MessageEvent<PickDateAndFormat>) => {
        ...
        port.postMessage({
          date: formattedDate,
          format,
          path: createDiaryPath(profile?.nickname ?? 'unknown', formattedDate),
  ...
  React.useEffect(() => {
    eventEmitter.on(EmitterEvent.SYNC, async () => {
      const diaries = await Promise.all(
        (await localforage.keys()).map(async (key) => ({
          ...
          content: await editor.blocksToHTMLLossy(await loadContent(key))
        }))
      )
    ...
    eventEmitter.on(EmitterEvent.EXPORT_DIARY, async (value: ExportParam) => {
      console.log(value)
      ...
      async (exportParam: ExportParam) => {
        ...
        const fileItems = await Promise.all(
          (await localforage.keys())
            .filter((key) => extractDataByDiaryKey(key)?.userId === profile?.id)
            .map(async (key) => ({
              path: join(
                exportParam.dir,
    ...
    eventEmitter.on(
      ...
      async (value: EditorContentData) => {
        ...
        await saveContent(
          createDiaryKey(
            profile?.id ?? 0,
            formatDateTime(value.date, DateTimeFormatEnum.DATE_FORMAT)
          ),
          value.shouldBeOverridden
            ? await parseEditorContent(value.format, value.contentToBeImported)
    ...
    eventEmitter.on(
      ...
      async (importAllParams: ImportAllParam) => {
        ...
        const toBeCreated: FinalImportsData['toBeCreated'] = await Promise.all(
          fileItemEntries
            .filter((entry) => !keys.includes(entry.key))
            .map(async (entry) => ({
              date: extractDataByDiaryKey(entry.key)?.date!,
        ...
        window.electronAPI.onPureRedirect<FinalImportsData>(async (value) => {
          try {
            for (const item of value.toBeCreated) {
              await saveContent(
                createDiaryKey(profile?.id ?? 0, item.date),
                await editor.tryParseMarkdownToBlocks(item.contentToBeImported)
              )
            }
              ...
              await saveContent(
                ...
                await editor.tryParseMarkdownToBlocks(
                  item.type === 'OVER_RIDE'
                    ...
                    : combineEditorContent(importAllParams.format, {
                        previous: item.contentToBeOverridden,
                        current: item.contentToBeImported
                      })
                )
              )
            }
      ...
      async (entries: SearchResultEntry[]) => {
        const markDownEntries = (await Promise.all(
          entries.map(async (entry) => ({
            ...
            mdText: await editor.blocksToMarkdownLossy(entry.diaryValue)
          }))
